...
## 開発上の注意
VisualStudioのエラーで、MainWindow.xamlの部品を配下のcsが読み込めない問題が発生することがあるが、これはアプリケーションでWindows FormsとWPFが同時に有効になっているからなので、一時的にWindows Formsを無効にしてWPFの部品として認識させた後に、再度有効にすることでこのエラーを回避できる。
尚、Windows Formsを有効にしている理由は、タスクトレイにアイコンを表示するAPIがWindows Formsにしか存在していないからである。(Microsoftさん、タスクトレイのAPIをWPFに追加してください)
